...
export async function apiFetch<T>(
  ...
  try {
    ...
    throw new Error(
      "Network request failed. Check connectivity or DNS resolution."
    );
  }
  ...
  if (!isJsonResponse) {
    console.warn("[apiFetch] non-JSON success response", {
      url,
      contentType,
    });
...
function safeErrorMessage(
  ...
  if (isJson) {
    try {
      ...
      console.warn("[apiFetch] failed to parse error body as JSON", {
        url,
        parseError,
      });
    }
